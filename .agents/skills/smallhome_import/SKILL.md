...
| 微信 URLs tracking | `weixin-urls.txt` (project root) |
| 微信 conversion script | `scripts/convert_weixin.ps1` |
| 微信 raw downloads | `weixin-articles/{title}/article.md` (auto-generated by `autocli weixin download`) |
...
| 微信公众号 | `wechat-{topic}` | `wechat-50sqm-loft.md` |
...
## Workflow F: Import from 微信公众号 (WeChat)

### Step 1: Find articles via Sogou WeChat Search

WeChat public articles are indexed by Sogou. Search with:

```powershell
# Search for keywords — returns list of article URLs
autocli sogou search "小户型装修 2026" --limit 20 --format json
```

Or use `WebFetch` on Sogou WeChat search:
```powershell
# type=2 limits to articles, type=1 for public accounts
WebFetch "https://weixin.sogou.com/weixin?query=小户型装修&type=2"
```

**⚠️ Anti-spider**: Sogou frequently returns a CAPTCHA/redirect page. If `WebFetch` fails, use `autocli read` (routes through Chrome with login session) to bypass.

### Step 2: Download article content

Use the AutoCLI weixin download command:

```powershell
autocli weixin download "https://mp.weixin.qq.com/s/{ARTICLE_ID}"
```

This downloads the article as markdown + all images to:
```
weixin-articles/{article-title}/
├── article.md                  # Markdown content with frontmatter
└── images/
    ├── img_001.jpeg            # Body images (may have #imgIndex=N suffix)
    ├── img_002.jpeg
    └── ...
```

**Troubleshooting**: If `autocli weixin download` isn't available, use the generic reader:
```powershell
autocli read "https://mp.weixin.qq.com/s/{ARTICLE_ID}" -f markdown
```

### Step 3: Track articles in weixin-urls.txt

Maintain a `weixin-urls.txt` in the project root to track all imported WeChat articles:

```
{article-title}|{slug-descriptor}
小户型装修的5个建议，至少别反着装|小户型装修5个建议
50㎡loft小户型，空间利用到极致！|50平loft空间利用
```

Format: `{title}|{short-slug-description}` — the title is used for directory lookup, the slug descriptor helps create the filename.

### Step 4: Run the conversion script

```powershell
pwsh scripts/convert_weixin.ps1
```

The script (at `scripts/convert_weixin.ps1`) does the following automatically:

1. **Reads each article** from `weixin-articles/{title}/article.md`
2. **Maps metadata** via pre-configured hash tables:
   - `$slugMap` — title → slug (e.g., `wechat-50sqm-loft`)
   - `$descMap` — title → SEO description
   - `$catMap` — title → categories array (`["area", "space"]`)
   - `$tagMap` — title → tags array (`["50㎡", "loft", "空间利用"]`)
3. **Copies images** from `weixin-articles/{title}/images/` to `public/images/articles/` with renamed filenames: `wechat-{slug}_img_{NNN}.{ext}`
4. **Generates frontmatter** with title, description, publishedAt, categories, tags, coverImage, author
5. **Rewrites image paths** in the body from relative (`images/img_001.jpeg`) to absolute (`/images/articles/wechat-slug_img_001.jpeg`)
6. **Writes** to `src/content/articles/zh/wechat-{slug}.md`

**⚠️ Before running the script**, you must add new entries to all four hash tables (`$slugMap`, `$descMap`, `$catMap`, `$tagMap`) for any new article title. These are at the top of `scripts/convert_weixin.ps1`.

### Step 5: Image cleanup and quality control

After conversion, the following issues commonly need manual fixes:

#### 5a. Remove `#imgIndex=N` from filenames

AutoCLI sometimes appends `#imgIndex=0` to image filenames. Fix with:

```powershell
Get-ChildItem "public/images/articles/wechat-*#*" | Rename-Item -NewName { $_.Name -replace '#.*$', '' }
```

Also remove all `#imgIndex=N` references from the markdown files:
```powershell
Get-ChildItem "src/content/articles/zh/wechat-*.md" | ForEach-Object {
    (Get-Content $_.FullName -Raw) -replace '#imgIndex=\d+', '' | Set-Content $_.FullName -Encoding UTF8
}
```

#### 5b. Fix missing img_001 references

The conversion script copies the first image separately as the cover image (`wechat-slug.ext`), but articles reference `wechat-slug_img_001.ext`. Create symlinks or copies:

```powershell
Get-ChildItem "public/images/articles/wechat-*.jpg" | Where-Object { $_.Name -match '^wechat-(.+)\.(jpe?g|png)$' -and $_.Name -notmatch '_img_' } | ForEach-Object {
    $base = $_.BaseName
    $ext = $_.Extension
    $img1 = Join-Path $_.Directory "${base}_img_001${ext}"
    if (-not (Test-Path $img1)) { Copy-Item $_.FullName $img1 }
}
```

#### 5c. Remove decorative images (<100px)

WeChat articles often include small icons, emojis, and decorative separators (24×17, 55×55, 62×60px). Remove them:

```powershell
# Using Python/PIL to check dimensions
pip install Pillow
python -c @'
import os
...
for f in os.listdir(dir):
    if not f.startswith("wechat-"): continue
    path = os.path.join(dir, f)
    try:
        img = Image.open(path)
        w, h = img.size
        if w < 100 or h < 100:
            print(f"REMOVE: {f} ({w}x{h})")
            # os.remove(path)
    except: pass
'@
```

After removing small images, remove their markdown references:
```powershell
python -c @'
import os, re
for f in os.listdir("src/content/articles/zh"):
    if not f.startswith("wechat-"): continue
    path = os.path.join("src/content/articles/zh", f)
    content = open(path, encoding="utf-8").read()
    # Find all image references, check if file exists
    refs = re.findall(r"\!\[image\]\((/images/articles/[^)]+)\)", content)
    for ref in refs:
        fname = ref.replace("/images/articles/", "")
        img_path = os.path.join("public/images/articles", fname)
        if not os.path.exists(img_path):
            print(f"  REMOVE REF: {fname} from {f}")
            content = content.replace(f"![image]({ref})", "")
    open(path, "w", encoding="utf-8").write(content)
'@
```

#### 5d. Replace poor-quality cover images

WeChat articles often use narrow banner images as covers (e.g., 640×107, 756×81). Replace with the best photo-quality image from the same article:

```powershell
# Find covers with poor aspect ratio or low resolution
python -c @'
import os
from PIL import Image
dir = "public/images/articles"
for f in os.listdir(dir):
    if not f.startswith("wechat-") or "_img_" in f: continue
    path = os.path.join(dir, f)
    try:
        img = Image.open(path)
        w, h = img.size
        ratio = w / h if h > 0 else 0
        if w < 500 or h < 300 or ratio > 3 or ratio < 0.5:
            print(f"POOR COVER: {f} ({w}x{h}, ratio={ratio:.1f})")
    except: pass
'@
```

Replace covers manually by:
1. Looking at the article's body images (wechat-slug_img_NNN.ext)
2. Finding the best photo-quality image (largest dimensions, good composition)
3. Copying it over the cover file

### Step 6: Build verification

Always verify the build succeeds after import:
...
| Error | Cause | Fix |
|-------|-------|-----|
| `Rollup failed to resolve import "images/img_005.jpeg"` | Image path not rewritten | Check regex in `convert_weixin.ps1`, re-run conversion |
| `Could not load content entry` | Frontmatter schema violation | Verify `categories` is array, not string |
| Missing image 404 warnings | Image file missing | Check file exists in `public/images/articles/` |
| ReferenceError in Layout | Locale code path issue | Verify `path` declaration before `locale` in Layout.astro |

---

## Workflow G: Batch Processing Pipeline

For bulk imports of 10+ articles from a single source:

### G1: WeChat Batch Import

```powershell
# 1. Collect article URLs in weixin-urls.txt (title|description format)
# 2. Download each one
foreach ($line in Get-Content weixin-urls.txt) {
    $title, $slugDesc = $line -split '\|'
    Write-Host "Downloading: $title"
    autocli weixin download "https://mp.weixin.qq.com/s/{get-id-somehow}"
}

# 3. Add entries to hash tables in scripts/convert_weixin.ps1
# 4. Run conversion
pwsh scripts/convert_weixin.ps1
...
Typical image counts for imported content:
- Bilibili: 1-3 images per video (cover + screenshots)
- Xiaohongshu: 5-15 images per note
- WeChat: 15-40 images per article (many are decorative)

For WeChat specifically, expect ~30% of images to need cleanup:
- ~15% are decorative elements under 100px → remove
- ~10% are narrow banners → replace cover
- ~5% are duplicates or irrelevant → consider removing
